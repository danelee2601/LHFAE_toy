...
class TrainPLM(pl.LightningModule):
    def __init__(self,
        ...
        self.track_vars = {}
        # self.train_x = None
        # self.train_recons_l = None
        # self.train_recons_h = None

    def forward(self, *args):
        return self.model(*args)

    def _store_tracking_data(self, batch_idx, x, recons_l, recons_l_min, recons_l_max, recons_h,):
        if batch_idx in [0, 1, 2, 3]:
            if batch_idx == 0:
                self.track_vars['x'] = x.cpu()
                self.track_vars['recons_l'] = recons_l.detach().cpu()
                self.track_vars['recons_l_min'] = recons_l_min.detach().cpu()
                self.track_vars['recons_l_max'] = recons_l_max.detach().cpu()
                ...
                self.track_vars['recons_l_min'] = torch.cat((self.track_vars['recons_l_min'], recons_l_min.detach().cpu()), dim=0)
                self.track_vars['recons_l_max'] = torch.cat((self.track_vars['recons_l_max'], recons_l_max.detach().cpu()), dim=0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        recons_l, recons_l_min, recons_l_max, z_l_mu, z_l_sigma, recons_h = self.model(x)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self._store_tracking_data(batch_idx, x, recons_l, recons_l_min, recons_l_max, recons_h)
